export const SITE_TITLE = 'Pete Watters';
export const SITE_DESCRIPTION =
  'Web3 frontend engineer with ten years shipping crypto products — wallets, trading terminals and institutional interfaces across Bitcoin, Stacks, Solana and EVM chains.';

export const ROUTES = {
  HOME: '/',
  BLOG: '/blog',
  CV: '/cv',
} as const;

export const GITHUB = {
  USERNAME: 'pete-watters',
  REPOS: ['leather-io/extension', 'leather-io/mono'],
} as const;

export const SOCIAL = {
  GITHUB: 'https://github.com/pete-watters',
  X: 'https://x.com/petew_btc',
  LINKEDIN: 'https://www.linkedin.com/in/pete-watters/',
  STACKOVERFLOW: 'https://stackoverflow.com/users/1365580/peadar',
} as const;
